...
export default class AbbreviationCommand extends Command {
  refresh(): void {
    ...
    if (firstRange && firstRange.isCollapsed) {
      if (selection.hasAttribute('abbreviation')) {
        ...
        const abbreviationRange = findAttributeRange(selectionFirstPosition, 'abbreviation', attributeValue, model)
  ...
  execute({ abbr, title }: { abbr: string, title: string }): void {
    const model = this.editor.model
    const selection = model.document.selection
